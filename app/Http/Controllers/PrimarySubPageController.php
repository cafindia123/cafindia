...
class PrimarySubPageController extends VoyagerBaseController
{
    ...
    public function edit(Request $request, $id)
    {
		...
		if($slug == "primary-page-sections"){
			$view = "voyager::$slug.edit-custom";
			// dd(compact('dataType', 'dataTypeContent', 'isModelTranslatable'));
			return Voyager::view($view, compact('dataType', 'dataTypeContent', 'isModelTranslatable'));
		}else if($slug == "primary-page-section-cards"){
    ...
    public function update(Request $request, $id)
    {
        ...
        if($request->template_type == 'CUSTOM' || $request->template_type == 'CUSTOM-LAYOUT') {
            $request['columns_in_rows'] = $request->columns_in_rows1;
        }
        $slug = $this->getSlug($request);
    ...
    public function store(Request $request)
    {
        ...
        if($request->template_type == 'CUSTOM-CARDS' || $request->template_type == 'CUSTOM-LAYOUT') {
            $request['columns_in_rows'] = $request->columns_in_rows1;
        }
		
		if($request->segments()[1] == "featured-stories"){
			$slug = $this->getSlug($request);
		}else{
			$slug = $request->segments()[2];//$this->getSlug($request);//"primary-page-sections";
		}
        $dataType = Voyager::model('DataType')->where('slug', '=', $slug)->first();
    ...
    {
        $slug = $this->getSlug($request);
		$primary_page_id = $request->get('primary_page_id');
        $dataType = Voyager::model('DataType')->where('slug', '=', $slug)->first();
        ...
        $results = $model->where("primary_page_id",$primary_page_id)->orderBy($dataType->order_column, $dataType->order_direction)->get();
    ...
    public function relation(Request $request)
    {
        ...
        foreach ($rows as $key => $row) {
            if ($row->field === $request->input('type')) {
				...
				if($slug =='users' && $options->model == 'TCG\Voyager\Models\Role'){
					$role_id = Auth::user()->role_id;
				  if($role_id == ROLE_ID_DEVELOPER || $role_id == ROLE_ID_ADMIN)
					{
					   $model = $model->whereNotIn('id',[ROLE_ID_DEVELOPER]);  
					}else{
						 $model = $model->whereNotIn('id',[ROLE_ID_DEVELOPER,ROLE_ID_ADMIN]);
					}
				}
