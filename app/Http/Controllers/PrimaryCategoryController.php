...
use Validator;
class PrimaryCategoryController extends VoyagerBaseController
{
    ...
    public function store(Request $request)
    {
		...
		if($slug == "primary-categories"){
			$v = Validator::make($request->all(), [
				'slug' => \Illuminate\Validation\Rule::unique('primary_categories')->where(function ($query) use($parent_id,$slug) {
					return $query->where('parent_id', $parent_id)->where('slug', $slug);
				})			
			],
			)->validate();
		}
		if($slug == "categories"){
			$v = Validator::make($request->all(), [
				'slug' => \Illuminate\Validation\Rule::unique('categories')->where(function ($query) use($parent_id,$slug) {
					return $query->where('parent_id', $parent_id)->where('slug', $slug);
				})			
			],
			)->validate();
    ...
    public function order(Request $request)
    {
        ...
        $model = app($dataType->model_name)->whereNull("parent_id");
        if ($model && in_array(SoftDeletes::class, class_uses_recursive($model))) {
            $model = $model->withTrashed();
        }
        $results = $model->orderBy($dataType->order_column, $dataType->order_direction)->with('children')->get();
    ...
    public function update_order(Request $request)
    {
        ...
        foreach ($order as $key => $item) {
            foreach($item->children as $k => $subItem) {
                if ($model && in_array(SoftDeletes::class, class_uses_recursive($model))) {
                    $j = $model->withTrashed()->findOrFail($subItem->id);
                } else {
                    $j = $model->findOrFail($subItem->id);
                }
                array_push($arr['child'], $subItem);
                $j->$column = ($k + 1);
                $j->save();    
            }
            if ($model && in_array(SoftDeletes::class, class_uses_recursive($model))) {
