...
class FooterSubPageController extends VoyagerBaseController
{
    ...
    {
        if(isset($request->layout_style))
            $request['layout_style'] = (int) $request->layout_style;
    ...
    {
        $slug = $request->segments()[2];//$this->getSlug($request);//"footer-page-sections";
        $dataType = Voyager::model('DataType')->where('slug', '=', $slug)->first();
    ...
    {
        $slug = $this->getSlug($request);
		$footer_page_id = $request->get('footer_page_id');
        $dataType = Voyager::model('DataType')->where('slug', '=', $slug)->first();
        ...
        $results = $model->where("footer_page_id",$footer_page_id)->orderBy($dataType->order_column, $dataType->order_direction)->get();
