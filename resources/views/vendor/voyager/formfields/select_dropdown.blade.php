        ...
        <select class="form-control select2" name="{{ $row->field }}">
            ...
            if (method_exists($dataTypeContent, $relationshipListMethod)) {
                ...
                if (isset($options->relationship->where)) {
                    ...
                    $relationshipOptions = $relationshipClass::get();
                }
            }
			...
			<?php
			$route = "";
				if( $relationshipOption->category_children_parent && $relationshipOption->category_children_parent->category_parent && $relationshipOption->category_children_parent->category_parent->category_parent ){
					$route = $relationshipOption->category_children_parent->category_parent->category_parent->name." -> ".$relationshipOption->category_children_parent->category_parent->name." -> ".$relationshipOption->category_children_parent->name." -> ";
				}else if( $relationshipOption->category_children_parent && $relationshipOption->category_children_parent->category_parent ){
					$route = $relationshipOption->category_children_parent->category_parent->name." -> ".$relationshipOption->category_children_parent->name." -> ";
				}else if( $relationshipOption->category_parent ){
					$route = $relationshipOption->category_parent->name." -> ";
				}
			?> 
                <option value="{{ $relationshipOption->{$options->relationship->key} }}" @if($default == $relationshipOption->{$options->relationship->key} && $selected_value === NULL) selected="selected" @endif @if($selected_value == $relationshipOption->{$options->relationship->key}) selected="selected" @endif>{{$route}} {{ $relationshipOption->{$options->relationship->label} }} </option>
            @endforeach
            </optgroup>
        </select>
    @else
