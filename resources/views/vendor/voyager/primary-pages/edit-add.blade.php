...
@section('content')
    <div class="page-content container-fluid">
        <form class="form-edit-add" role="form" action="@if($edit){{ route('voyager.primary-pages.update', $dataTypeContent->id) }}@else{{ route('voyager.primary-pages.store') }}@endif" method="POST" enctype="multipart/form-data">
                ...
                <div class="col-md-8">
                    ...
                    <div class="panel">
						...
						<h3 class="panel-title">
                            ...
                            </h3> <div class="panel-actions">
                                <a class="panel-action voyager-angle-down" data-toggle="panel-collapse" aria-hidden="true"></a>
                            </div>
                        </div>
                        <div class="panel-body">
                           <?php
										$query = app('App\Models\PrimaryCategory')::with("category_children_parent");
									?>
							@if($edit)
								<?php
							...
							<div class="form-group">
                               <h4 class=""> <label for="category_id">Navigation Pages</label> </h4>
                               ...
                               <h4 class=""> <label for="category_id">Navigation Pages</label> </h4>
								...
								$query->whereNotExists(function ($query) {
												$query->select(\DB::raw(1))
												  ->from('primary_pages')
												  ->whereRaw('primary_pages.category_id = primary_categories.id');
												});
								?>

                                <select class="form-control select2" name="category_id">
									<option value="">Please Select Main Navigation Page</option>
                            ...
                            <div class="form-group">
                                <h4 class=""><label for="heading">Heading</label> </h4>
                                @include('voyager::multilingual.input-hidden', [
                                    '_field_name'  => 'body_heading',
                                    '_field_trans' => get_field_translations($dataTypeContent, 'body_heading')
                                ])
                                <input type="text" class="form-control" id="body_heading" name="body_heading" placeholder="Heading" value="{{ $dataTypeContent->body_heading ?? '' }}">
							</div>
                        </div>
                    </div>
                    ...
                    <div class="panel">
                        ...
                        <div class="panel-body">
                            @php
                                $dataTypeRows = $dataType->{($edit ? 'editRows' : 'addRows' )};
                                $exclude = ['title', 'body', 'excerpt', 'slug', 'status', 'category_id', 'author_id', 'featured', 'image', 'meta_description', 'meta_keywords', 'seo_title', 'body_heading'];
                            @endphp
...
@if($edit)
            <div class="row">
				...
				<a href="{{ route('voyager.primary-page-sections.order',['primary_page_id'=>$dataTypeContent->id] ) }}" class="btn btn-primary btn-order-new" style="margin-left: 24px;">
                    <i class="voyager-list"></i> <span>{{ __('voyager::bread.order') }}</span>
                        ...
                        <div class="panel-body">
                            @php
                               $query = app('App\Models\PrimaryPageSection')::with("card_children")->where("primary_page_id",$dataTypeContent->id)->orderBy("order","asc")->get();
								$dataTypeRows = $query;
                            @endphp
...
<table class="fold-table table table-hover">
 ...
 <tbody id="showhide_{{$row->id}}___{{$row->id}}" style="background: rgb(234, 234, 234); display: none;">
	...
	<tr>
        ...
        <td><a href="{{ route('voyager.primary-page-section-cards.edit', $card_row->id) }}" title="Edit" class="btn btn-sm btn-primary pull-right edit primary-page-section-cards">
		<i class="voyager-edit"></i><span class="hidden-xs hidden-sm">Edit</span>
		<a href="{{ route('voyager.primary-page-section-cards.destroy', $card_row->id) }}" title="Delete" class="btn btn-sm btn-danger pull-right delete-page-section" data-id="{{$card_row->id}}" id="delete-{{$card_row->id}}">
...
<td>{{ $row->bg_style }}</td>
	...
	<a href="{{ route('voyager.primary-page-sections.edit', $row->id) }}" title="Edit" class="btn btn-sm btn-primary pull-right edit primary-page-sections">
		<i class="voyager-edit"></i><span class="hidden-xs hidden-sm">Edit</span>
	</a>
	<a href="{{ route('voyager.primary-page-sections.destroy', $row->id) }}" title="Delete" class="btn btn-sm btn-danger pull-right delete-page-section" data-id="{{$row->id}}" id="delete-{{$row->id}}">
            <i class="voyager-trash"></i> <span class="hidden-xs hidden-sm">Delete</span>
        </a>
											@if($row->template_type != "MEDIA" && $row->template_type != "CUSTOM_LAYOUT")

												<div title="View" class="btn btn-sm btn-warning view install_card {{ $row->template_type }} " data-id="{{$row->id}}">
													<i class="voyager-plus"></i> Add Page Sections Crad
												</div>
											@endif
										</td>
									 </tr>
			...
			{{-- @include('voyager::featured-stories.list') --}}
			@endif
<!---------------------------------------------------------------->
	...
	<div class="modal modal-success fade" tabindex="-1" id="install_modal" role="dialog" data-backdrop="static" data-keyboard="false">
        <div class="modal-dialog">
            <form class="modal-content" action="{{route('primary-page-sections.store')}}" id="install_form" method="POST" enctype="multipart/form-data">
                <input type="hidden" name="primary_page_id" value="{{$dataTypeContent->id}}">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="title" class="form-control" id="titled" placeholder="Title of hook">
                    </div>
					<div class="form-group">
						<label class="control-label" for="description">Description</label>
						<textarea class="form-control PageSectionsrichTextBox" name="description" rows="5" id="descriptionPageSections"></textarea>
					</div>
					...
					<div class="form-group section-is_custom_layout2">
						...
						<input  type="text" class="form-control" name="link2" placeholder="Link Url" value="">
						<span class="glyphicon glyphicon-question-sign" aria-hidden="true" data-toggle="tooltip" data-placement="right" data-html="true" title="Link Format Like This :- https://exm.com OR https://www.exm.com"></span>
					</div>
					<div class="form-group section-is_custom_layout2" >
						<label class="control-label" for="link2_text">Second Link Title</label>
						<input  type="text" class="form-control" name="link2_text" placeholder="Card Link Title1" value="">
					</div>
					<div class="form-group section-is_custom_layout2">
	...
	<div class="modal modal-success fade" tabindex="-1" id="install_modal_card" role="dialog" data-backdrop="static" data-keyboard="false">
        <div class="modal-dialog">
            <form class="modal-content" action="{{route('primary-page-sections.store-cards')}}" id="install_form" method="POST" enctype="multipart/form-data">
                <input type="hidden" name="primary_page_section_id" id="primary_page_section_id">
                <input type="hidden" name="status" value="A">
				<div class="modal-header">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        <label for="title">Title</label>
                        <input type="text" name="title" class="form-control" id="titledd" placeholder="Title of hook" required>
                    </div>
					<div class="form-group">
						<label class="control-label" for="description">Description</label>
						<textarea  class="form-control PageSectionsrichTextBox" name="description" rows="5" id="descriptionPageSectionsCard"></textarea>
					</div>
	...
	<div class="modal modal-success fade" tabindex="-1" id="install_modal_card_layout" role="dialog" data-backdrop="static" data-keyboard="false">
        <div class="modal-dialog">
            <form class="modal-content" action="{{route('primary-page-sections.store-custom-layout')}}" id="install_form1" method="POST" enctype="multipart/form-data">
                <input type="hidden" name="primary_page_section_id" id="primary_page_section_id1">
                <input type="hidden" name="status" value="A">
				<div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                                aria-hidden="true">&times;</span></button>
                ...
                <div class="modal-body">
					<div class="form-group">
						<label for="image_icon">Image/Icon</label>
						<select class="form-control select2" name="image_icon" id="image_icon1" required>
							<option value="1">Heading</option>
							<option value="2">Inner Card</option>
							<option value="3">Inner Media</option>
							<option value="4">Link</option>
						</select>
					</div>
                    <div class="form-group">
                        <label for="title">Title</label>
                        <input type="text" name="title" class="form-control" id="titledd1" placeholder="Title of hook" required>
                    </div>
					<div class="form-group">
						<label class="control-label" for="description">Description</label>
						<textarea  class="form-control PageSectionsrichTextBox" name="description" rows="5" id="descriptionPageSectionsCard1"></textarea>
					</div>
					<div class="form-group section-is_layout22">
						<label class="control-label" for="image">Image</label>
						<input  type="file" name="image" accept="image/*">
...
$(document).on('click','.Featured-page-sections', function(e){
	e.preventDefault();
	$form = $(this);
	$.ajax({
		url : $form.attr('href'),
		...
		success : function(data) {
			...
			setTimeout( function(){
			$('.form-group.hidden.col-md-12 input[name="primary_page_id"]').val($form.data("id"));
			console.log($('.form-group.hidden.col-md-12 input[name="primary_page_id"]'));
			},1000);

		$(document).ready(function() {
            var additionalConfig = {
                selector: '#edit_modal textarea.richTextBox',
            }
...
/*************************************************************/
$(document).on('click','.primary-page-sections', function(e){
	e.preventDefault();
	$form = $(this);
	$.ajax({
		url : $form.attr('href'),
		...
		success : function(data) {
			...
			if( $('select#template_type1.form-control.select2').val() == "MEDIA" ){
				 ...
				 $(".form-group.section-is_custom_layout2").hide();
				 $(".form-group.section-is_layout22card").hide();
				 $(".form-group.section-is_layout22custom").hide();
			 } else if($('select#template_type1.form-control.select2').val() == "CUSTOM_LAYOUT" ){
				 $(".form-group.section-is_layout2").show();
				 $(".form-group.section-is_custom_layout2").show();
				 $(".form-group.section-is_layout22card").hide();
				 $(".form-group.section-is_layout22custom").hide();
			 } else {
				 $(".form-group.section-is_layout2").hide();
				 $(".form-group.section-is_custom_layout2").hide();
                 if( $('select#template_type1.form-control.select2').val() == "CARDS" ) {
                     $(".form-group.section-is_layout22card").show();
                     $(".form-group.section-is_layout22custom").hide();
                 } else {
                     $(".form-group.section-is_layout22card").hide();
                     $(".form-group.section-is_layout22custom").show();
                 }
			 }
			 $('select#template_type1.form-control.select2').change( function(){
				 ...
				 if( $thisPrivent.val() == "MEDIA" ){
				    ...
				    $(".form-group.section-is_custom_layout2").hide();
					 $(".form-group.section-is_layout22card").hide();
				    $(".form-group.section-is_layout22custom").hide();
				 } else if($thisPrivent.val() == "CUSTOM_LAYOUT") {
					 $(".form-group.section-is_layout2").show();
				    $(".form-group.section-is_custom_layout2").show();
					 $(".form-group.section-is_layout22card").hide();
				    $(".form-group.section-is_layout22custom").hide();
...
$(document).on('click','.primary-page-section-cards', function(e){
	e.preventDefault();
	$form = $(this);
	$.ajax({
		url : $form.attr('href'),
		...
		success : function(data) {
			$("#edit_modal .modal-dialog").html(data);
			$("#edit_modal").modal("show");  $(document).ready(function() {
            var additionalConfig = {
                selector: 'textarea.PageSectionsrichTextBox1',
            }
...
$(document).on('click','[id^=eachDeliveryMens_]', function(e){
	...
	$('td.bread-actions').on('click', '.install_card', function (e) {
		var form = $(this);
        if($(this).hasClass('CUSTOM-LAYOUT')) {

        }
		$("#primary_page_section_id").val( form.data("id") );
	...
	if( $('select#template_type.form-control.select2').val() == "MEDIA" ){
        ...
        $(".form-group.section-is_layout22card").hide();
        $(".form-group.section-is_layout22custom").hide();
	 } else if( $('select#template_type.form-control.select2').val() == "CUSTOM_LAYOUT" ) {
        $(".form-group.section-is_layout2").show();
        $(".form-group.section-is_custom_layout2").show();
        $(".form-group.section-is_layout22card").hide();
        $(".form-group.section-is_layout22custom").hide();
        ...
        $(".form-group.section-is_layout2").hide();
        $(".form-group.section-is_custom_layout2").hide();
        if( $('select#template_type.form-control.select2').val() == "CARDS" ){
            $(".form-group.section-is_layout22card").show();
            $(".form-group.section-is_layout22custom").hide();
        } else {
            $(".form-group.section-is_layout22card").hide();
