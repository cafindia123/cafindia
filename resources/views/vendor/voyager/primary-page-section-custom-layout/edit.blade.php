<form class="modal-content" action="{{route('voyager.'.$dataType->slug.'.update', $dataTypeContent->getKey())}}" id="install_form" method="POST" enctype="multipart/form-data">{{ method_field("PUT") }}
	...
	<div class="modal-body">
		<div class="form-group">
			<label for="contact_type">Content Type</label>
			<select class="form-control select2" name="contact_type" id="contact_type" required>
				<option value="1" @if($dataTypeContent->contact_type == "1") selected @endif>Heading</option>
				<option value="2" @if($dataTypeContent->contact_type == "2") selected @endif>Inner Cards</option>
				<option value="3" @if($dataTypeContent->contact_type == "3") selected @endif>Inner Media</option>
				<option value="4" @if($dataTypeContent->contact_type == "4") selected @endif>Link</option>
			</select>
		</div>
		<div class="form-group">
			<label for="title">Title</label>
			<input type="text" name="title" class="form-control" id="titledd" placeholder="Title of hook" required value="{{$dataTypeContent->title}}">
		</div>
		<div class="form-group">
